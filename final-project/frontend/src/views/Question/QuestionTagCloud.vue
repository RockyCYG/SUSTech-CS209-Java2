<template>
  <div class="">
    <div id="myChart1" style="height: 1000px;">
...
onMounted(() => {
  request.get("/stackoverflow/chart/wordcloud", {
    params: {
...
const getContainerFontFamily = (chart: echarts.ECharts) => {
...
const initChart = () => {
  ...
  const option = {
    title: {
      show: true,
      text: "The most popular 500 Java topics in StackOverflow",
      x: "center",
      y: "top",
      textAlign: null,
      textStyle: {
        fontFamily: getContainerFontFamily(myChart),
        fontSize: 40,
        fontStyle: 'italic',
        fontWeight: 'bold'
      }
    ...
    series: [
      {
        name: "The most 5000 popular tags",
        type: "wordCloud",
        keepAspect: false,
        sizeRange: [15, 300],
        rotationRange: [-50, 50],
        ...
        left: "center",
        top: "center",
        width: Math.min(Math.min(myChart.getWidth(), myChart.getHeight()) * 1, 1000),
        height: Math.min(
          Math.min(myChart.getWidth(), myChart.getHeight()) * 0.8,
          800
        ),
        gridSize: 8,
        ...
        data: tagList.sort((a, b) => b.value - a.value),
      },
    ],
  };
